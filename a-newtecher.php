...
if (isset($_GET['delete_prn'])) {
    ...
    // Perform the deletion of the faculty's data
    $deletefaculty = $conn->prepare("DELETE FROM faculty WHERE prn = ?");
    $deletefaculty->bind_param("s", $delete_prn);

    if ($deletefaculty->execute()) {
        // Successful deletion
        header('location: a-newtecher.php');
        exit();
    } else {
        // Error in deletion
        echo "Error deleting faculty: " . $deletefaculty->error;
    }
...
if (isset($_GET['verify_prn'])) {
    ...
    $updateStatus = $conn->prepare("UPDATE faculty SET status = 'verified' WHERE prn = ?");
    ...
    if ($updateStatus->execute()) {
        ...
        // Error in update
        $message= "Error updating faculty status: " . $updateStatus->error;
    }
...
<head>
    ...
    <title>New Faculty Registrations</title>
...
<body>
    <div class="sidebar">
        ...
        <ul>
                ...
                <li>
                    <a href="a-newtecher.php"  class="active"><span><i class="las la-chalkboard-teacher"></i></span>New Teachers</a>
                </li>
                <li>
    ...
    <div class="main-content">
        ...
        <div class="card">
                <div class="card-header">
                    <h3>Teachers/Faculty</h3>
                </div>
                    <div class="card-body">
                            <table id="example" width="100%">
                                <?php
                                $addmissions = mysqli_query($conn, "
                                SELECT name, prn, department, email, mobile, status FROM faculty WHERE status = 'pending'") or die('query failed');
                                ?>
                                <thead>
                                    <tr>
                                        <td>Name</td>
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href='a-newtecher.php?delete_prn=<?php echo $fetch['prn']; ?>' onclick="return confirm('Are you sure you want to delete this faculty?');">
                                                <i class="las la-trash-alt"></i>
                                                </a>
                                                <a class="verify" href='a-newtecher.php?verify_prn=<?php echo $fetch['prn']; ?>'>verify</a>
                                            </td>
